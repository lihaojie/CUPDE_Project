...
public static class NativeGallery
{
	...
	public delegate void GetSomethingCallback( string json, int action );
	public delegate void PermissionCallback( int result, int action );
	public delegate void MediaPickMultipleCallback( string[] paths );
	public delegate void GetIPhoneSomethingCallback( string path );
	...
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern void _NativeGallery_PickCameraImage( string imageSavePath );
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern void _NativeGallery_PickCameraVideo( string imageSavePath );
	...
	// action 2.位置
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern int _NativeGallery_CheckLocation();
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern int _NativeGallery_RequestLocation();
	
	// action 3.麦克风 
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern int _NativeGallery_CheckMicrophone();
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern int _NativeGallery_RequestMicrophone();
	
	// action 4.相机 
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern int _NativeGallery_CheckCamera();
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern int _NativeGallery_RequestCamera();


	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern void _NativeGallery_GetSSID();
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern void _NativeGallery_GetVersionBuild();
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern void _NativeGallery_GetBuild();
	
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern void _NativeGallery_GetCacheSize();
	...
	public static Permission CheckIPhonePermission(int action)
	{
		...
		{
			return (Permission) _NativeGallery_CheckLocation();
		}
		else if(action == 3)
		{
			return (Permission) _NativeGallery_CheckMicrophone();
		}
		else if(action == 4)
		{
			return (Permission) _NativeGallery_CheckCamera();
		}
	...
	// action 1.读取相册 2.位置 3.麦克风 4.相机 
	public static Permission RequestIPhonePermission(int action)
	{
#if !UNITY_EDITOR && UNITY_IOS
		if (action == 1){
		...
		{
			return (Permission) _NativeGallery_RequestLocation();
		}
		else if (action == 3)
		{
			return (Permission) _NativeGallery_RequestMicrophone();
		}
		else if (action == 4)
		{
			return (Permission) _NativeGallery_RequestCamera();
		}
		return Permission.Granted;
#else
		return Permission.Granted;
	...
	// action 1.SSID  2.版本号 + build 号 3.绑定设备 4.build 5.cacheSize

	public static void GetSomethingFromIPhone(MediaPickCallback callback, int action)
	{
	...
	public static Permission GetLocationSSIDCallback(MediaPickCallback callback)
	{
		return GetLocationSSIDFromGallery(callback);
	}
	

	public static void GetImageFromGalleryForAndroid( MediaPickCallback callback, string title = "", string mime = "image/*", bool crop = true, int action = 0 )
	{
		if (Application.platform == RuntimePlatform.Android)
		{
			GetMediaFromGallery( callback, MediaType.Image, mime, title, crop, action);
		}
	}
	
	public static void GetVideoFromGalleryForAndroid( MediaPickCallback callback, string title = "", string mime = "video/*" , int action = 0 )
	{
		if (Application.platform == RuntimePlatform.Android)
		{
			GetMediaFromGallery( callback, MediaType.Video, mime, title , false, action);
			
		}
	}

	
	public static Permission GetIPhoneCameraImageFromGallery( MediaPickCallback callback, string title = "", string mime = "image/*" )
	{
		return GetIPhoneCameraMediaFromGallery( callback, MediaType.Image, mime, title );
	}
		
	public static Permission IPhoneCameraMakeVideoFromGallery( MediaPickCallback callback, string title = "", string mime = "image/*" )
	{
		return IPhoneCameraMakeVideoMediaFromGallery( callback, MediaType.Image, mime, title );
	}
	 ...
	 * 1、定位，2、相册，3、拍照，4、联网，5、版本信息
	 */
	public static void GetSomethingFromNative(GetSomethingCallback callback, int action, string message = null)
	{
#if !UNITY_EDITOR && UNITY_ANDROID
			AJC.CallStatic( "GetSomethingFromNative", Context, new NGMediaReceiveCallbackAndroid( callback ), action, message);
#elif !UNITY_EDITOR && UNITY_IOS
			// TODO ios todo something
#else
			if( callback != null )
...
// action 2相册，3拍照
	...
	// iPhone - 拍照
	private static Permission GetIPhoneCameraMediaFromGallery( MediaPickCallback callback, MediaType mediaType, string mime, string title )
	{
...
#elif !UNITY_EDITOR && UNITY_IOS
			NGMediaReceiveCallbackiOS.Initialize( callback );
			_NativeGallery_PickCameraImage(SelectedImagePath);
#else
			if( callback != null )
				callback( null );
#endif
	...
	private static Permission GetMultipleMediaFromGallery( MediaPickMultipleCallback callback, MediaType mediaType, string mime, string title, bool crop = true, int action = 0 )
	{
		Permission result = RequestPermission( false );
		if( result == Permission.Granted && !IsMediaPickerBusy() )
		{
			if( CanSelectMultipleFilesFromGallery() )
			{
				...
				AJC.CallStatic( "PickMedia", Context, new NGMediaReceiveCallbackAndroid( null, callback ), (int) mediaType, true, savePath, mime, title, crop, action );
#else
				if( callback != null )
					callback( null );
#endif
